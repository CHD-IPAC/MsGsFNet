...
def parse_args():
    ...
    parser.add_argument('--weights', default='//media/ubuntu/2ad0bba9-bef3-4ad8-b5ea-812c019ea962//zwq//oold//pretrainedweights//_bestMiou_//HongHu//small_ablation_only_spatial in rate of 25.pt', type=str, help='path to weights file. For resnet101/152, ignore this arg to download from torchvision')
    parser.add_argument('--save_path', default='//media/ubuntu/2ad0bba9-bef3-4ad8-b5ea-812c019ea962//zwq//oold//pretrainedweights//The ERF_matrix//x2.npy', type=str, help='path to save the ERF matrix (.npy file)')
    parser.add_argument('--num_images', default=1, type=int, help='num of images to use')   #  指定num_images，计算多少张图片的热力图的平均值，并保存
    ...
    parser.add_argument('--model', default='small_ablation_only_spatial', help='Network a  rchitecture?')
...
def main(args):
    ...
    if args.hsi_c == 'rad' or args.hsi_c == 'ref':
        ...
        trainset = HongHuLoader(set_type='seed10_25_train_aug', hsi_mode='all', transforms=transform,
                                augs=None)  # first_train second_train third_train fourth_train fifth_train
        valset = HongHuLoader(set_type='NEW_seed10_25_test', hsi_mode='all', transforms=transform)
        trainloader = data.DataLoader(trainset, batch_size=args.batch_size, shuffle=True)
        valloader = data.DataLoader(valset, batch_size=args.batch_size, shuffle=False)
