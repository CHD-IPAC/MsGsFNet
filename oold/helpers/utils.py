...
class Metrics():
    ...
    def __call__(self, truth, prediction):            # len(prediction) = 123904
        # 有out of index报错的多半是因为你的模型预测的像素点数和真实的像素点个数不相同
        ...
        prediction = np.delete(prediction, ignore_locs)     # 471040-59400=411640      会根据 ignore_locs 数组中的索引值，删除 prediction 数组中对应位置的元素
        # print(np.unique(prediction))
        # print('prediction after delate:',prediction.shape)
        self.c = confusion_matrix(truth, prediction)
        hun = self.c
        print('second hunxiaojuzhen shape', hun.shape)#second hunxiaojuzhen shape (16, 16)
        # print('second hunxiaojuzhen:\n',hun)
        # return self._oa(), self._aa(), self._mIOU(), self._dice_coefficient(), self._IOU()
        return self._oa(), self._aa(), self._mIOU(), self._dice_coefficient(), self._IOU(), self._f1Score(), self._precision(), self._recall(), self._FWIoU()
    ...
    def _f1Score(self):
        ...
        recall = np.diag(self.c) / np.sum(self.c, axis=1)
    ...
    def _FWIoU(self):
        ...
        np.seterr(divide="ignore", invalid="ignore")
        freq = np.sum(self.c, axis=1) / np.sum(self.c)  # Frequency of each class
        iou = self._IOU()  # IoU per class
        # Ignore the classes with zero frequency
        FWIoU = np.sum(freq * iou)  # Weighted sum of IoU
        return FWIoU

class LongKouLoader(data.Dataset):

    def __init__(self, set_type='train', size='32', hsi_sign='rad',
...
class QingyunLoader(data.Dataset):
    def __init__(self, set_type='train', size='32', hsi_sign='rad',
                 hsi_mode='all', transforms=None, augs=None):
        self.set_type = set_type
        # if set_loc == 'right' and set_type == 'test':
        ...
        # self.working_dir = osp.join('XiongAn', self.working_dir, 'Data-' + set_loc)
        self.working_dir = 'Image' + size +'_step_patch'#overlap 3127
        self.working_dir = osp.join('QUH-Qingyun', self.working_dir)
    ...
    def __getitem__(self, index):                        # 在创建了一个LongKouLoader对象之后，eg主函数中：当你使用 enumerate(trainloader, 0) 对 trainloader 进行迭代时，
        ...
        label = imageio.v2.imread(osp.join(self.working_dir, self.label_dir, self.filelist[index] + '.tif'))
        # label = cv2.imread(osp.join(self.working_dir, self.label_dir, self.filelist[index] + '.tif'))
        # label = label[:, :]
        # print('-1 zhihou type', type(label))#<class 'numpy.ndarray'>
        # print('-1 zhihou', label.shape)#!!!!!!!!!!!!!!!!!!!!!!!1 (64, 64, 3)
