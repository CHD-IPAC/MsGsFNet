    ...
    for i in range(1, len(name_saved) + 1):
        ...
        if np.any(label_one_chann):  # if not all ground  至少有一类了 排除纯背景
            ...
            if (15 in unique_values) and globals()['trainlei15'] <= int(lei15zongshu * percent2):
                for lei in range(17):  # 假设有 23 个类别
                ...
                train_3.write("%s\n" % name)
                continue
            elif (12 in unique_values) and globals()['trainlei12'] <= int(lei12zongshu * percent2):
                for lei in range(17):  # 假设有 23 个类别
                    if lei in unique_values:
                        globals()[f'trainlei{lei}'] += np.sum(label_one_chann == lei)
                train_3.write("%s\n" % name)
                ...
                train_3.write("%s\n" % name)
                continue
            elif (7 in unique_values) and globals()['trainlei7'] <= int(lei7zongshu * percent2):
                for lei in range(17):  # 假设有 23 个类别
                    if lei in unique_values:
                        globals()[f'trainlei{lei}'] += np.sum(label_one_chann == lei)
                train_3.write("%s\n" % name)
                continue
            elif (10 in unique_values) and globals()['trainlei10'] <= int(lei10zongshu * percent2):
                for lei in range(17):  # 假设有 23 个类别
                    if lei in unique_values:
                        globals()[f'trainlei{lei}'] += np.sum(label_one_chann == lei)
                train_3.write("%s\n" % name)
...
if __name__ == "__main__":
    ...
    labels = io.imread(osp.join(folder_dir, 'WHU-Hi-HanChuan_gt.tif'))[1:, 15:]
    ...
    # print(f"有效标注比例：{valid_ratio:.2f}%")  # 68.95%
    ...
    def process_txt_file(file_path, output_file_path):
        ...
        # 调用函数
    process_txt_file(
        r'E:\Something\oold-take new loss-XiongAn-master_noleft_concision_two 7×7s\WHU-Hi-HanChuan\Image32_step_patch\NEW_seed10_{}_train.txt'.format(
            int(percent * 100)),
        r'E:\Something\oold-take new loss-XiongAn-master_noleft_concision_two 7×7s\WHU-Hi-HanChuan\Image32_step_patch\NEW_seed10_{}_train_aug.txt'.format(
            int(percent * 100)))
